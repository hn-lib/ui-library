package com.henong.android.widget.calendar.component;
...
import com.henong.android.widget.calendar.theme.ADCircleDayTheme;

/**
 * 
 * 
 * FILE: ADCircleMonthView.java
 * 
 * MODULE: com.henong.android.widget.calendar.component
 * 
 * PURPOSE: TODO
 * 
 * AUTHOR(S): Joker.Chen
...
public class ADCircleMonthView extends MonthView {
	...
	protected void drawBG(Canvas canvas, int column, int row, int day) {
		...
		float radius = columnSize < (rowSize * 0.6) ? columnSize / 2
				: (float) (rowSize * 0.6) / 2;
	...
	@Override
	protected void drawDecor(Canvas canvas, int column, int row, int year,
			int month, int day) {
	...
	@Override
	protected void drawRest(Canvas canvas, int column, int row, int year,
			int month, int day) {
		if (calendarInfos != null && calendarInfos.size() > 0) {
			float radius = columnSize < (rowSize * 0.6) ? columnSize / 2
					: (float) (rowSize * 0.6) / 2;
			for (CalendarInfo calendarInfo : calendarInfos) {
				if (calendarInfo.day == day && calendarInfo.year == year
						&& calendarInfo.month == month + 1) {
					float restX = columnSize * column
							+ (columnSize + paint.measureText(day + "")) / 2;
					float restY = rowSize * row + rowSize / 2
	...
	@Override
	protected void drawText(Canvas canvas, int column, int row, int year,
			int month, int day) {
		paint.setTextSize(theme.sizeDay());
		float startX = columnSize * column
				+ (columnSize - paint.measureText(day + "")) / 2;
		float startY = rowSize * row + rowSize / 2
				- (paint.ascent() + paint.descent()) / 2;
		paint.setStyle(Paint.Style.STROKE);
		...
		if (day == selDay) {// 日期为选中的日期
			if (!TextUtils.isEmpty(des)) {// desc不为空的时候
				...
				paint.setTextSize(theme.sizeDesc());
				int desX = (int) (columnSize * column + (columnSize - paint
						.measureText(des)) / 2);
				int desY = (int) (rowSize * row + rowSize * 0.9 - (paint
						.ascent() + paint.descent()) / 2);
...
//				canvas.drawText(des, desX, desY, paint);
			...
			if (!TextUtils.isEmpty(des)) {// 没选中，但是desc不为空
				...
				int desX = (int) (columnSize * column + Math
						.abs((columnSize - paint.measureText(des)) / 2));
