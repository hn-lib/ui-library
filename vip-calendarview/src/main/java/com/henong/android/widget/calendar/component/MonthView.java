...
public abstract class MonthView extends View {
	...
	private ISmoothListener smoothListenner;
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		// TODO there is a bug that will automatic change the rowSize
		// int heightSize = (int) (NUM_ROWS * rowSize);
		int heightSize = NUM_ROWS * 100;
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_UP:
			...
			if (upX - downX > 0 && Math.abs(upX - downX) > mTouchSlop * 10) {// 左滑
				...
				smoothListenner.afterSmoothedEvent();
	...
	public void setSmoothListener(ISmoothListener smoothListener) {
		this.smoothListenner = smoothListener;
	}
	...
	public void computeScroll() {
		if (mScroller.computeScrollOffset()) {
			...
			//i don't know why that there is so much invalidate(),it's may cause low performance...
//			invalidate();
		}
	}
	...
	protected void doClickAction(int x, int y) {
		...
		callbackDefaultChoosen();
	}

	protected void callbackDefaultChoosen(){
		if (dateClick != null) {
	...
	private void computeDate() {
		...
		//after computeDate we should triggering callback the click event to front.
		callbackDefaultChoosen();
	}
